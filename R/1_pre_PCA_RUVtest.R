...
quant_RUVg <- newSeqExpressionSet(
  counts = counts(quant_big),
...
groups <- str_c(
  as.data.frame(colData(quant_big))$cell_line,
...
quant_RUVs <- newSeqExpressionSet(
  counts = counts(quant_big),
  phenoData = colData(quant_big) %>% as.data.frame()
) %>%
...
plotPCA_paired <- function(
  ...
  ggplot(data = d, aes_string(x = pcs[1], y = pcs[2], color = "group")) +
    ...
    xlab(paste0(
      pcs[1],
      ": ",
      round(percentVar[pcsToUse[1]] * 100),
      "% variance"
    )) +
    ylab(paste0(
      pcs[2],
      ": ",
      round(percentVar[pcsToUse[2]] * 100),
      "% variance"
    )) +
    coord_fixed() +
...
counts(quant_big_batchcor) <- sva::ComBat_seq(
  counts(quant_big_batchcor),
