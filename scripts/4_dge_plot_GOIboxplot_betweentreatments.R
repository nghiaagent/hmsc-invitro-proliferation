...
ggplot(df_plot,
  ...
  scale_x_discrete(labels = c("Treated" = "Hep",
                              "Untreated" = "Ctrl"))
...
plots_rt2array <- map(geneIds_rt2array[geneIds_rt2array %in% as.character(quant_DGE_small$genes$ENTREZID)],
                           plot_GOI)
...
for (i in 1:length(plots_rt2array)) {
  
  message(str_c("drawing plot for ",
                names(plots_rt2array)[[i]],
                sep = ""))
  
  ggsave(
    filename = str_c(names(plots_rt2array)[[i]],
                     ".png",
                     sep = ""),
    plot = plots_rt2array[[i]],
    path = file.path(
      '.',
      'output',
      'plots_boxplot_logCPM',
