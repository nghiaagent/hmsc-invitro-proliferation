...
msigdb_GOCC <- getGmt(con = "./input/genesets/msigdb_v2023.2.Hs_GMTs/c5.go.cc.v2023.2.Hs.entrez.gmt")

## MSigDB

msigdb_h  <- getGmt(con = "./input/genesets/msigdb_v2023.2.Hs_GMTs/h.all.v2023.2.Hs.entrez.gmt")

msigdb_c2 <- getGmt(con = "./input/genesets/msigdb_v2023.2.Hs_GMTs/c2.all.v2023.2.Hs.entrez.gmt")

msigdb_c3 <- getGmt(con = "./input/genesets/msigdb_v2023.2.Hs_GMTs/c3.all.v2023.2.Hs.entrez.gmt")

## ReactomePA
## Represented by MSigDB c2/CP/Reactome

msigdb_reactome <- getGmt(con = "./input/genesets/msigdb_v2023.2.Hs_GMTs/c2.cp.reactome.v2023.2.Hs.entrez.gmt")
...
lapply(fit_GSVA, decideTests) %>%
  lapply(summary) %>%
  lapply(function(x) x[,c(25:30, 37:39)])

## Venn diagrams (Treat x Day)

lapply(fit_GSVA, decideTests) %>%
  lapply(summary) %>%
  lapply(function(x) x[,c(7:12)])

## Venn diagrams (Treat x Passage)

lapply(fit_GSVA, decideTests) %>%
  lapply(summary) %>%
  lapply(function(x) x[,c(13:24)])
