...
quant_cDNA_DGE <- readRDS(file = "./output/data_expression/pre_DGE/quant_refseq_DGEList.RDS")
...
ggsave(
  "./output/plots_QC/Library size - RefSeq + limma-voom.png",
  plot = plot_libsize,
  width = 12,
  height = 6,
  scale = 0.7
...
lcpm_pre_TMM <-
  cpm(quant_cDNA_DGE[keep_cDNA_edgeRfiter, , keep.lib.sizes=FALSE],
      log = TRUE) %>%
  as_tibble() %>%
  mutate(ENTREZID = rownames(quant_cDNA_DGE_filter)) %>%
...
ggsave(
  "./output/plots_QC/distribution RefSeq.png",
...
png("./output/plots_QC/Density of count values - Refseq.png", width = 10, height = 30, units = 'cm', res = 600) 
...
lines(den$x, den$y, col=col[i]) } 
abline(v=log.cutoff, col="red", lwd=1, lty=2, main="") 
title("Filtered (edgeR 20 counts)",xlab="log2-CPM") 
...
png(
  "./output/plots_QC/Sample correlation heatmap - Refseq.png",
  width = 60,
  height = 60,
  units = 'cm',
  res = 300
...
saveRDS(quant_cDNA_DGE_filter, file = "./output/data_expression/pre_DGE/quant_refseq_DGEList_filter.RDS")
