...
Unsupervised clustering using Principal Component Analysis (PCA) was used for data exploration. In this context, PCA reduces the dimensionality of our datasets from thousands of genes to a smaller number of principal components (PCs). Here, each PC is uncorrelated from each other, the first PCs (PC1, PC2, etc.) are composed of the most highly variable genes in the dataset, and together, the PCs retain most of the original dataset’s variance. Additionally, expression heatmaps using DEGs to summarise differential expression in the dataset.
...
-   DEGs are extracted with t-statistics (think expression fold change but normalised for dataset standard deviation)
